...
export default function AdminVideos() {
  ...
  const handleVimeoConvert = () => {
    const embedCode = newRow.embed_code || '';
    
    // VimeoのURL判定
    const vimeoUrlMatch = embedCode.match(/vimeo\.com\/(\d+)/);
    if (vimeoUrlMatch) {
      const videoId = vimeoUrlMatch[1];
      const convertedCode = `<div style="padding:177.78% 0 0 0;position:relative;"><iframe src="https://player.vimeo.com/video/${videoId}?badge=0&amp;autopause=0&amp;player_id=0&amp;app_id=58479&amp;autoplay=1&amp;controls=0&amp;title=0&amp;byline=0&amp;portrait=0" frameborder="0" allow="autoplay; fullscreen; picture-in-picture; clipboard-write" style="position:absolute;top:0;left:0;width:100%;height:100%;" title=""></iframe></div>`;
      setNewRow(r => ({ ...r, embed_code: convertedCode }));
      return;
    }
    
    // 既存の埋め込みコードにパラメータ追加
    if (embedCode.includes('player.vimeo.com')) {
      const updatedCode = embedCode.replace(
        /src="([^"]+)"/,
        'src="$1&amp;autoplay=1&amp;controls=0&amp;title=0&amp;byline=0&amp;portrait=0"'
      );
      setNewRow(r => ({ ...r, embed_code: updatedCode }));
    }
  };
    ...
    for (const id of ids) {
      ...
      if (editData) {
        await supabase.from('videos').update({
          ...
          url: editData.embed_code !== undefined && editData.embed_code.trim() === '' ? '' : videos.find(v => v.id === videoId)?.url,
        }).eq('id', videoId);
      }
    }
